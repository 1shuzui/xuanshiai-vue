...
1. **产品口径冲突。** 当前 `AGENTS.md` 的硬边界与 `../最终版的文字需求/定版决策收口记录.md` v2.0 在认证、收藏/喜欢、会员与爆灯范围上不一致。
...
| 主题 | 当前 `AGENTS.md` 硬边界 | 现有定版记录 / 旧计划口径 | 本计划处理 |
|---|---|---|---|
| 认证门槛 | 实名与单身承诺书是本期门槛 | 双重认证 = 实名 + 学历 | **Phase 0 阻断项。** 本计划按当前 `AGENTS.md` 约束，不新增“实名 + 学历即通过门槛”的实现；先同步根目录 PRD |
| 收藏 / 喜欢 | 收藏仅自己可见；喜欢是轻意向 | 喜欢 = 仅自己可见列表 | **拆成两个动作。** 禁止继续用一个 `liked` 状态承载两种语义 |
| 会员 / VIP / 爆灯 | 本期不把会员订阅购买、VIP 开通引导或爆灯做成主路径 | 原计划列入 P2 主交付 | **移出本期关键路径。** 不新增购买闭环、强引导或爆灯主入口 |

在上述冲突完成权威文档同步前：

- 可以进行代码审计、状态梳理、无争议的安全修复、组件复用和响应式修复。
- 不应新增会员购买、VIP 引导、爆灯支付或以付费换取沟通权限的实现。
...
1. 用户本次任务的明确授权。
2. `../最终版的文字需求/定版决策收口记录.md`；若与当前 `AGENTS.md` 的硬边界冲突，先收口并更新权威 PRD，不在代码中自行裁决。
3. `../最终版的文字需求/` 中未冲突的页面文字、结构和截图。
4. `AGENTS.md` 的生产、安全、配置与验证约束。
...
| 认证 | 实名与单身承诺书是本期门槛；状态必须真实，审核中不能显示为通过 |
| 收藏 | 私有行为，仅自己可见 |
| 喜欢 | 轻意向；不得与收藏共用同一文案和状态 |
...
- [ ] 实名与单身承诺书作为本期门槛；学历等其他认证只按已确认用途展示。
- [ ] 不允许审核中状态点亮“已完成门槛”。
...
| `../最终版的文字需求/定版决策收口记录.md` | 定版 PRD 决策主副本；冲突需先收口 |
