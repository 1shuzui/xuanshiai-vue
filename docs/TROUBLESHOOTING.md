...
1. 先取消或跳过关联提示并重新编译。
<<<<<<< HEAD
2. 确认 `API_CONFIG.useHttp` 或目标模块 Mock 开关符合本次验证场景。
=======
...
<<<<<<< HEAD
4. 开发期 `uniCloud: false`；当前全局走 HTTP，部分模块走显式 Mock。若提示关联云服务空间，可取消/跳过，不要删 `uniCloud-aliyun/`。
=======
